...
const props = defineProps({
  ...
  // В встречах режим с единой лентой должен быть упрощён: оставляем только
...
<template>
  <div class="quick-toolbar" :class="{ compact }">
    ...
    <!-- Именованный слот сразу после Группировка/Пузырьки — сюда QuickFiltersBar
         вставляет статус (Все/Не выполнено/Выполнено) и исполнителей, чтобы они
         шли непосредственно за этой парой кнопок, а не после сортировки/спейсера. -->
    <slot name="after-view-mode" />
