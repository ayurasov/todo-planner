...
const occurrenceBadgeLabel = computed(() => {
  if (props.task.meetingTitle || props.task.occurrenceDate) {
    return [
      props.task.meetingTitle,
      props.task.occurrenceDate ? formatDateTime(props.task.occurrenceDate) : null,
    ].filter(Boolean).join(' · ')
  }
    ...
    >
      ...
      <div class="task-main">
        ...
        <div v-if="!editingTitle" class="task-meta">
          ...
          <span
            v-else-if="occurrenceBadgeLabel"
            class="tag occurrence-badge"
            :title="occurrenceBadgeLabel"
          ><AppIcon name="repeat" :size="11" /> {{ occurrenceBadgeLabel }}</span>
          ...
          <span v-if="prefs.showLastUpdatedDate && task.updatedAt" class="date-meta" :title="`Последнее изменение: ${task.updatedAt}`">
