...
from app.models import (
    ListMembershipORM, ListORM, ManagerDepartmentORM, MeetingAttendeeORM,
    MeetingORM, TaskORM, TaskTagORM, TaskWatcherORM, UserORM,
)
...
class PermissionService:
    ...
    def get_managed_department_ids(self, user_id: str):
        if not user_id:
            return []
        return [row.department_id for row in ManagerDepartmentORM.query.filter_by(user_id=user_id).all()]

    def manages_department(self, user_id: str, department_id) -> bool:
        return bool(department_id and department_id in self.get_managed_department_ids(user_id))
    ...
    def is_meeting_attendee(self, meeting_id: str, user_id: str) -> bool:
        if not meeting_id or not user_id:
            return False
        return MeetingAttendeeORM.query.filter_by(meeting_id=meeting_id, user_id=user_id).first() is not None
    ...
    def can_view_list(self, list_id: str, user_id: str) -> bool:
        ...
        return role is not None or self.manages_department(user_id, self._list_department_id(list_id))

    def can_create_task(self, list_id: str, user_id: str) -> bool:
        if self.is_global_admin(user_id):
            return True
        role = self.get_role(list_id, user_id)
        return role in CAN_EDIT_ANY_TASK or self.manages_department(user_id, self._list_department_id(list_id))

    def can_edit_task(self, task: d.Task, user_id: str) -> bool:
        if self.is_global_admin(user_id):
            return True
        if not task.list_id:
            return task.created_by == user_id or task.assignee_id == user_id or self.manages_department(user_id, self._user_department_id(task.assignee_id))
        role = self.get_role(task.list_id, user_id)
        return (role in CAN_EDIT_ANY_TASK or (role == LIST_ROLE_ASSIGNEE and task.assignee_id == user_id)
                or self.manages_department(user_id, self._list_department_id(task.list_id))
                or self.manages_department(user_id, self._user_department_id(task.assignee_id)))
    ...
    def can_manage_members(self, list_id: str, user_id: str) -> bool:
        if self.is_global_admin(user_id):
            return True
        return self.get_role(list_id, user_id) in CAN_MANAGE_MEMBERS

    def can_delete_list(self, list_id: str, user_id: str) -> bool:
        if self.is_global_admin(user_id):
            return True
        return self.get_role(list_id, user_id) in CAN_DELETE_LIST

    def can_delete_task(self, task: d.Task, user_id: str) -> bool:
        if self.is_global_admin(user_id) or task.created_by == user_id:
            return True
        if not task.list_id:
            return self.manages_department(user_id, self._user_department_id(task.assignee_id))
        role = self.get_role(task.list_id, user_id)
        return role in CAN_EDIT_ANY_TASK or self.manages_department(user_id, self._list_department_id(task.list_id))

    def get_accessible_list_ids(self, user_id: str):
        if self.is_global_admin(user_id):
            return sorted(row.id for row in ListORM.query.all())
        ids = {row.list_id for row in ListMembershipORM.query.filter_by(user_id=user_id).all()}
        ids.update(row.id for row in ListORM.query.filter(ListORM.department_id.in_(self.get_managed_department_ids(user_id))).all())
        return list(ids)

    def is_task_visible(self, task: d.Task, role=None, user_id=None, is_global_admin=False) -> bool:
        if is_global_admin or task.created_by == user_id:
            return True
    ...
    def can_view_task_via_department(self, task: d.Task, user_id: str) -> bool:
        ...
        return bool(managed_department_ids and (
            (task.list_id and self._list_department_id(task.list_id) in managed_department_ids)
            or self._user_department_id(task.assignee_id) in managed_department_ids
            or self._user_department_id(task.created_by) in managed_department_ids
        ))
    ...
    def get_analytics_scope_user_ids(self, user_id: str):
        ...
        scope.update(row.id for row in UserORM.query.filter(UserORM.department_id.in_(self.get_managed_department_ids(user_id))).all())
        return scope
    ...
    def get_task_domain(self, task_id: str):
        ...
        if row is None:
            return None
        watcher_ids = [row.user_id for row in TaskWatcherORM.query.filter_by(task_id=task_id).all()]
        tags = [row.tag for row in TaskTagORM.query.filter_by(task_id=task_id).all()]
        task = orm_to_domain.task(row, watcher_ids=watcher_ids, tags=tags)
...
def require_list_permission(permission_method_name: str, list_kwarg: str = "list_id"):
    def decorator(view):
        ...
        def wrapper(*args, **kwargs):
            if not getattr(permission_service, permission_method_name)(kwargs.get(list_kwarg), current_user_id()):
...
def require_task_permission(permission_method_name: str, task_kwarg: str = "task_id"):
    def decorator(view):
        ...
        def wrapper(*args, **kwargs):
            ...
            task = permission_service.get_task_domain(kwargs.get(task_kwarg))
            if task is None:
                return jsonify({"error": "not_found", "message": "Задача не найдена"}), 404
            if not getattr(permission_service, permission_method_name)(task, user_id):
